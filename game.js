class Game {
    ...
    startGame() {
        if (this.inputBid.value < 1) return alert(`kwota jest za mala!`);
        const bid = Math.floor(this.inputBid.value);

        if (!this.wallet.checkCanPlay(bid)) {
            return alert(`masz za malo srodkow lub zostala podana nieprawidlowa wartosc! :C`);
        }
        ...
        this.stats.addGameToStatisc(win, bid)
